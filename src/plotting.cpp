...
void plotChebPoly(std::string &filename, std::vector<mpfr::mpreal> &coeff,
  ...
  std::function<mpfr::mpreal(mpfr::mpreal)> f =
    ...
    mpfr::mpreal result;
    evaluateClenshaw(result, coeff, x, prec);
    return result;
  };
  plotFunc(filename, f, a, b, prec);
...
void plotChebPolys(std::string &filename,
  ...
  std::vector<std::function<mpfr::mpreal(mpfr::mpreal)>> fs(coeffs.size());

  for (std::size_t i = 0u; i < coeffs.size(); ++i) {
    fs[i] = [&](mpfr::mpreal x) -> mpfr::mpreal {
      mpfr::mpreal result;
      evaluateClenshaw(result, coeffs[i], x, prec);
      return result;
    };
  }
  plotFuncs(filename, fs, a, b, prec);
